 ...
 * @author labszk
 */
@RegisterStorage(PCJSavitzkyGolay.Shared.class)

public class PCJSavitzkyGolay implements StartPoint
{
    @Storage(PCJSavitzkyGolay.class)
    ...
    {
        PCJ.start(PCJSavitzkyGolay.class, new NodesDescription("nodes.txt"));
    }
    ...
    public void main() throws Throwable
    {
        ...
        if(PCJ.myId() != 0)
        {
            PCJ.put(x[0], PCJ.myId() - 1, Shared.right, 0);
            PCJ.put(x[1], PCJ.myId() - 1, Shared.right, 1);
        }
        // prześlij prawą zakładkę do procesora z prawej strony
        if(PCJ.myId() != PCJ.threadCount() - 1)
        {
            PCJ.put(x[n - 2], PCJ.myId() + 1, Shared.left, 0);
            PCJ.put(x[n - 1], PCJ.myId() + 1, Shared.left, 1);
        }
